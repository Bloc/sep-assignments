...
RSpec.describe LinkedList, type: Class do
  let(:n1) { Node.new("Rob","Rob") }
  let(:n2) { Node.new("Ben", "Ben") }
  let(:n3) { Node.new("Mike", "Mike") }
  ...
  describe "#print" do
    ...
    specify { expect { llist.print }.to output("[Rob, Ben]\n").to_stdout }
  end
  ...
  describe "#delete" do
    ...
    it "removes the middle element of a list properly" do
      ...
      puts "now"
      llist.print
      puts "llist.head: #{llist.head.value}, n1: #{n1.value}"
      puts "llist.head.next: #{llist.head.next.value}, n3: #{n3.value}"
      puts "llist.tail: #{llist.tail.value}, n3: #{n3.value}"
